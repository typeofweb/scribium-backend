...
export class SchoolsController {
  ...
  @Get(':id')
  async getSchool(@Param('id', ParseIntPipe) id: number): Promise<SchoolDto> {
    return this.schoolsMapper.map(await this.schoolsService.getSchoolById(id));
  }

  @Post()
  async createSchool(@Body() createSchoolDto: CreateSchoolDto): Promise<SchoolDto> {
  ...
  @Delete(':id')
  async deleteSchool(@Param('id', ParseIntPipe) id: number): Promise<SchoolDto> {
